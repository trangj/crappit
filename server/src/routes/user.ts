...
router.post("/register", async (req, res) => {
	try {
		...
		res.status(200)
			.cookie('token', refresh_token, { httpOnly: true, maxAge: 1000 * 60 * 60 * 24 * 7, secure: true, sameSite: false })
			.json({
				access_token,
				user: newUser,
				status: {
...
router.post("/refresh_token", async (req, res) => {
	try {
		...
		res.status(200)
			.cookie("token", new_refresh_token, { httpOnly: true, maxAge: 1000 * 60 * 60 * 24 * 7, secure: true, sameSite: false })
