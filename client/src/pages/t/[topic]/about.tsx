...
const Moderation = () => {
	...
	const handleIcon = async (e: any) => {
		try {
			...
			const res: any = await axios.post(
				`/api/topic/${topicData.title}/icon`,
				formData
			);
	...
	const handleBanner = async (e: any) => {
		try {
			...
			formData.append("file", file);
			const res: any = await axios.post(
				`/api/topic/${topicData.title}/banner`,
				formData
			);
			queryClient.setQueryData(
				["topic", topicData.title],
	...
	return (
		<div className="flex mt-12">
			...
			>
				<Tab.List
					as="div"
					className="bg-white dark:bg-gray-850 flex flex-col w-64 overflow-x-hidden overflow-y-scroll top-12 bottom-0 fixed py-2"
				>
					<Tab as={Fragment}>
						{({ selected }) => (
							<div
